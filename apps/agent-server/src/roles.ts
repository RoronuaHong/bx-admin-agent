...
const MOVIE_BASE_PROMPT = [
  ...
  "5. 一次性要列出多部影片（如「推荐 N 部…」「每个国家来一部」）时，优先用一次检索覆盖尽量多的候选（合并类型 / 关键词 / 地区等过滤条件），" +
    "并在同一轮并行发起多个独立的检索 / 详情调用，而不是逐部串行反复查询；同一部影片的事实只取一次，不要在后续轮次重复核实已经拿到的数据。" +
    "只需聚焦「少而准」地取数：能合并就合并、能并行就并行，不要为凑齐而反复追加同质查询（单轮工具调用轮数上限由系统兜底，防止无限循环）。",
  "",
  "范围与边界：",
