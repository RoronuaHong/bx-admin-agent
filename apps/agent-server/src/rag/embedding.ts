...
export async function embedTexts(texts: string[]): Promise<EmbeddingResult[]> {
  ...
  return vectors.map((vector) => ({ vector, model: cfg.model }));
}
...
export async function embedTextsBatched(
  ...
  await Promise.all(
    Array.from({ length: Math.min(concurrency, Math.max(1, Math.ceil(texts.length / batchSize))) }, () => worker()),
  );
  return results;
}
