...
export const router = createRouter({
  ...
  routes: [
    ...
    { path: "/trace", component: () => import("./pages/TracePage.vue"), meta: { auth: true, loginPath: "/agents/admin/login", agent: "portal", requiredPermission: "canViewTrace" } },
...
router.beforeEach(async (to) => {
  ...
  if (to.meta.requiredPermission && !me.permissions?.[to.meta.requiredPermission]) {
    return {
      path: "/",
      query: {
        denied: String(to.meta.requiredPermission),
