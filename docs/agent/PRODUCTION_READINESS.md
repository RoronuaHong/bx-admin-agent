...
- **权限口径（2026-09-07 更新）**：`/trace` 是门户级路由，不再挂在 `/agents/admin/trace` 作为产品主入口；但页面入口展示、路由进入、HTTP 数据读取统一受 `canViewTrace` 控制。当前 `canViewTrace` 由服务端权限投影输出，可用 `TRACE_ALLOWED_OWNERS`（`countryId:loginName`）做白名单收紧；未配置时保持“已登录可看”。
...
| 2026-09-07 | Trace 门户权限收口 | ✅ `Trace` 提升为门户级路由 `/trace`；Header 入口按 `canViewTrace` 展示；前后端统一权限投影；支持 `TRACE_ALLOWED_OWNERS` 白名单收紧（未配则已登录可看） | src/app.ts / src/config.ts / .env.example / apps/web/src/router.ts / apps/web/src/pages/AgentPortalPage.vue / apps/web/src/pages/TracePage.vue |
...
4. **前端可视化** ✅ 已落地（2026-09-04，2026-09-07 权限收口）：Web `/trace`「调用观察」只读页——汇总 stats（含 emptyRoundRate / degradeHint）+ 最近 run 表 + 点选展开 span 树；门户 Header 入口按 `canViewTrace` 展示；支持 `TRACE_ALLOWED_OWNERS` 白名单收紧。
