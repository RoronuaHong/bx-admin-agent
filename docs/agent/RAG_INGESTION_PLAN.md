...
> 状态：方案已定；2026-09-01 全网调研细化分阶段方案（见 §8），待用户提供样本文档/文档目录后启动 P0
...
## 8. 2026-09-01 全网调研 + 分阶段实施方案（细化）

> 用户确认需求线：企业内部文档 → 知识库 → /chat 聊天查询。
> 当前待办：200+ 文档实际目录待用户更新；样本文档（见 §9）待用户提供。

### 8.1 全网调研结论（2026 年共识）

1. **文档解析 + 分块质量决定 RAG 80% 效果上限**（腾讯云 ADP 指南），大于模型选型与检索算法的影响
2. **分块第一原则：按结构切，不按字数切**（标题 > 段落 > 句子递归回退）；表格整块带表头，每块自解释（"复印机 3 台 1.2 万"不带列名无法判断 3 是数量还是单价）
3. **父子分块是进阶标配**：子块小（~200 字）负责检索，父块（4 倍大）供上下文——"检索靠小的，回答靠大的"
4. **混合检索 + Rerank 是生产级标配**：向量 0.6 + 关键词 0.3 + 元数据过滤 0.1；Top-K 召回后大模型重排显著提精度
5. **评估必做**：50-100 个代表性问题评测集（来自 Top 50 高频问题），改参数跑分说话，不拍脑袋
6. **解析工具选型（本地轻量向）**：docx→mammoth（JS 纯本地）；pdf 文本→unpdf（已装）；扫描件→MinerU/Docling 太重（1.5GB 模型/需 GPU）不划算，标记跳过；复杂 PDF 可选 MarkItDown（微软开源转 markdown）
7. **钉钉文档 OpenAPI 确认可行**：有「文档搜索」接口 + 异步「导出任务」回调 + **应用授权模式**（appKey/appSecret→token，无需逐用户授权）——方案 A 路线成立，只差管理员凭证

### 8.2 分阶段方案

| 阶段 | 内容 | 优先级 | 工作量 |
|---|---|---|---|
| 1 | 解析器注册表：walkDir 按扩展名分发（md/txt/html 现有；docx→mammoth 需新增依赖；xlsx/csv→exceljs 已装；pdf→unpdf 已装，扫描件标记跳过）；表格转文本强制带表头；先出文件类型清点报告 | P0 | 1-2 天 |
| 2 | 分块策略升级：结构感知递归切分（标题→段落→句子逐级回退）+ 10~20% 重叠；可选父子分块 | P0.5 | 小-中 |
| 3 | 检索增强：大模型 Rerank（Top-20 召回 → 精排取 5，零新依赖）；元数据过滤扩展；可选情境增强 Contextual Retrieval | P1 | 小 |
| 4 | 评估体系：50-100 题评测集 + verify 脚本跑分基线 | P1 必做 | 小 |
| 5 | 增量索引：md5/ctime 指纹，只重建变更文档 | P1 | 小-中 |
| 6 | 网页链接入库：普通网页正文抓取；摹客RP 走"导出 PDF/HTML → 本地通道"（不实时抓取） | P2 | 中 |
| 挂账 | 钉钉文档拉取（等管理员凭证）；权限过滤（按项目/角色） | - | - |

**执行顺序**：阶段 1 → 2 → 4（建基线）→ 3 → 5 → 6

**最快见效路径**：先清点文件类型 → 拿 5-10 篇真实文档跑通解析入库 → 立刻建评测集看基线 → 再逐项优化。

### 8.3 当前待办

> 2026-09-02 用户指示：先把计划/待办写进文档固化，暂不开工；等文档目录/样本文档到位后再启动实施。

**依赖用户侧：**
- [ ] 用户更新 200+ 文档实际目录（阶段 1 扫描根目录确认，默认 `docs/knowledge/`）
- [ ] 用户提供样本文档（见 §9）

**等文档期间可先开工（待用户确认后启动）：**
- [ ] 阶段 2 分块策略升级：结构感知递归切分（标题→段落→句子逐级回退）+ 10~20% 重叠 + 表格整块带表头（现有 6 篇 md 即可验证）
- [ ] 阶段 4 评测集框架：建 50-100 题评测集结构 + verify 脚本跑分基线
- [ ] 阶段 1 解析器注册表骨架：walkDir 按扩展名分发，md/txt 先接通，docx/xlsx/pdf 预留解析器接口

## 9. 样本文档需求清单（待用户提供）

| # | 类型 | 建议数量 | 验证重点 |
|---|---|---|---|
| 1 | .md / .txt（制度、规范类） | 2 | 标题层级（#/##/###）分段；长文档按结构切不切碎 |
| 2 | .docx（Word 制度/通知类） | 2 | mammoth 中文乱码；标题/段落/表格结构还原 |
| 3 | .xlsx（数据表/参数表） | 1-2 | 表格转文本带表头完整；合并单元格/多 sheet 处理 |
| 4 | .csv | 1 | 简单解析 + 表头识别 |
| 5 | .pdf 文本型（可选中文字） | 1-2 | unpdf 提取中文完整；多栏/表格是否错乱 |
| 6 | .pdf 扫描件（图片型） | 1 | 识别为"需 OCR"并正确跳过不报错 |

**内容覆盖建议**：
- 制度类：报销流程、考勤制度、差旅标准（验证条款切分）
- 流程/操作类：含步骤编号的操作手册（验证列表分段）
- 带表格文档：参数表、价目表、配置清单（验证表格带表头）
- 长文档 1 篇（≥10 页）：验证分块不碎、检索能命中中段内容
- 含英文/数字混合：型号、金额、日期（验证检索精确匹配）

**交付方式**：直接放到 `docs/knowledge/` 下按目录归类（人事/安全/财务/运维/产品 等），或放到任意目录后告知路径，扫描时指向它。
