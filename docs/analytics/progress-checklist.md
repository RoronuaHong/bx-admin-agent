...
| 对接 Metabase 数据库权限 | **✅ 已落地** | `src/analytics/metabase-client.ts`；db **2** 主库。口径是 **72 / 67 / 5**（总表 / 可答 / 隐藏）。隐藏规则：`metabase_upload` / `upload_*` / `*_tmp` / `*_dict`。`scripts/enumerate-metabase-tables.mjs` 打印同一口径，不再写 `coverage: 1 / N`。**coverage GATE**：`evaluateWarehouseCoverage` + `config/analytics/warehouse-coverage.json`；CI `test:analytics-gate` 失败则红。无 Metabase 时仍锁 overlay 1 表 / 8 字段 + 扩表名单。 |
...
| 整理数据 + 初步分析 | **✅ 核心已落** | `analyticsAsk`：NL → 护栏 → 目录 → 时间 → **schema linking（不是锁单表）** → Path A/B/C → 守卫 → 取数 → 对表 → 解读。问数页表格支持 **CSV** 导出；聊天侧才有 `export_dataset` xlsx。语义层扩表：overlay `elt_watch_detail` → VQR 五表 + **`gather_stat`（埋点汇总金样）** → 其余 batch 2 仍 catalog_only。 |
| 人工校验 | **✅ 代码已落，门禁未全绿当验收** | `analytics-eval-harness.mjs` + soft-EX 1.5%。EX / CWR / RefuseRecall / RefusePrecision **以及 warehouse coverage** 分列：72/67/5 **不是** GATE EX 分母，但 **coverage 漂移会使 GATE 失败**（即使 EX 全绿）。Path C `llm_sql` **不计 EX**。refuse-only 可 100%；全量 EX 不保证绿。 |
...
已修：巡检日报 cron、enumerate 口径、对照文档、warehouse coverage GATE（72/67/5 + 扩表名单）、`gather_stat` 埋点汇总金样。
