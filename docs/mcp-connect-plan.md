...
>
> ⚠️ **2026-09-21 口径变更**：本文多处把「一个都不勾」写成**纯直连**，该说法已不再成立——内置工具（`fs_*` / `write_todos` / `task` / `read_skill` / `search_tools` / `render_chart` 等）**始终注入**，不勾任何连接器只表示**不接外部数据源**（本地出图、工作区文件、任务计划照常可用）。现状一律以 `docs/mcp-guide.md` 为准。
