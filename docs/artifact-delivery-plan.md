...
## 15. `fs_delete`：工作区删除工具（2026-09-24，P1 第 4 项之一）

### 15.1 为什么必须补（不只是「工具集对称」）

| 理由 | 依据 |
|---|---|
| **硬约束会变成死结** | `FS_MAX_FILES` 默认 **100**；`fsWrite` / `fsWriteBinary` 在达上限时直接返回「文件数已达上限（100）」。`fs_*` 只增不删 → 长对话、反复导出、大量卸载结果会把配额吃满，且**没有任何回收手段** |
| 能力不对称 | 模型能 `fs_write` 却永远不能清理，废弃草稿与中间产物无法回收 |
| 用户直述需求 | 「把那个文件删掉」是自然指令，目前只能靠 `run_command` 代劳（external、更危险） |

### 15.2 关键设计决策：删除**要过闸门**（因为不可逆）

`risk.ts` 对工作区写「免确认」的口径依据是（`verdictNeedsConfirm` 注释，2026-09-22）：
> 闸门只留给**不可逆 / 跨出信任边界**的动作……工作区写在沙箱内、路径与体积有上限，**属可逆的本地动作**。

- **删除不满足「可逆」这一前提** → 按同一条判据，它应当进闸门。
- 故 `fs_delete` 登记为 `destructive` + `workspace`，并把闸门判据覆盖 destructive：
  `verdictNeedsConfirm = level === "destructive" || (external && level !== "read")`。
- **连带修正一处潜在不一致**：`subagentMayExecute` 与 `verdictNeedsConfirm` 本应是「同一条判据的两面」，
  但前者只看 `external`——若不一起改，**子代理会绕过闸门执行需要确认的删除**（而子代理送不出确认事件，
  会挂到超时）。两者改为共用同一判据。
- `fs_write` / `fs_edit` **保持不变**（写仍可逆，维持免确认，避免确认疲劳）。

### 15.3 实现

- `fs-store.ts` 新增 `fsDelete(conversationId, path)`：`safePath` 越界拒绝 → 非普通文件 / 不存在时**明确报错** → `rmSync`。
- `builtins.ts`：`BUILTIN_RISK` 登记（`destructive` / `workspace`）+ spec + `execBuiltin` 分支。
- 工具描述引导：只删自己写过的中间文件 / 废弃草稿；**交付产物（`export_data` 产出）删除前须确认用户不再需要**；一次只删一个文件。
- **不加入 `WORKSPACE_FILE_WRITE_TOOLS`**：该集合语义是「**已免确认** → 靠审计兜底」（`write-gate` 用例 [C]
  据此校验「名单与登记表自洽」）。`fs_delete` 走确认闸门，确认动作本身就有
  `confirmed` / `denied` / `timeout` 留痕（`audit.ts`），混进去反而破坏该不变量——
  这一点在实现时踩过一次，故在此记明。
- **路径语义（与既有 `fs_*` 描述有出入，实测为准）**：`safePath` 对含 `..` 或盘符的路径**拒绝**，
  但对绝对路径是**剥离前导斜杠后关进工作区**（`/etc/passwd` → 工作区内 `etc/passwd`），并非拒绝。
  因此 `fs_write` / `fs_read` 等描述里「绝对路径一律被拒绝」的说法**与实现不符**——
  本次只修正 `fs_delete` 自己的措辞，其余工具的措辞统一列为挂账（见 §15.6）。

### 15.4 明确不做的边界

- **不做软删除 / 回收站**：既然走确认闸门，真删即可；软删除会让「配额仍被占用」，删了个寂寞。
- **不做批量 / 目录删除**：只删单个文件，降低误伤面（批量可由多轮调用完成，也避免一条命令清空工作区）。
- **不特判卸载产物**：`governToolResults` 卸载的大结果也在工作区，删掉后原句柄失效——
  模型侧表现为 `fs_read` 报「不存在」，而卸载文案本就是「需要该数据请重新调用对应工具」，可接受。

### 15.5 验证

`tests/fs-delete.test.ts`（5/5 通过）：
[A] 删除成功并回报释放字节数 · [B] 含 `..`/盘符被拒、绝对路径被关进工作区而非解析到系统路径 ·
[C] 不存在 / 目录时明确报错 · [D] 闸门判据：destructive 需确认、子代理不可执行、
工作区写仍免确认、只读放行、外部写仍确认 · [E] 风险登记与审计口径（不在「免确认」集合内）。

全量回归：**28 文件 / 167 测试全绿**（`write-gate` 用例 [C] 的不变量在修正后恢复通过）。

### 15.6 挂账

- `fs_write` 描述里的「绝对路径与 `..` 一律被拒绝」与 `safePath` 实际行为不符（实际是关进工作区，安全性没问题，措辞不准）：
  **已于 2026-09-24 修正**为「路径含 `..` 或盘符会被拒绝，写绝对路径时也会被限定在工作区内、不会碰到系统文件」，与 `fs_delete` 措辞统一。
  `fs_read` / `fs_edit` / `fs_ls` / `fs_glob` / `fs_grep` 经核查未含该不准措辞（`safePath` 行为见 `fs-store.ts:73-80`：`..`/盘符/`\` 拒绝，裸绝对路径 `resolve(root, raw)` 关进工作区）。
