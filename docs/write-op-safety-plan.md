...
- **第三轮补齐（同日）：跨进程留档 + 无进展看门狗**。①**留档落库**：`src/task-store.ts` 把「任务状态 + 事件缓冲 + 正文累积」按 `CHAT_TASK_FLUSH_MS`（默认 800ms）批量落 Mongo（集合 `chat_tasks`，TTL 默认 30 分钟自动清理；连不上 Mongo 退回进程内存），续传路由在内存没有该任务时退回留档回放——**重启后断线期间产出的事件一样能补齐**；收口语按留档**真实状态**给（`interrupted` 才说「未完成的部分请重新发起」，`success`/`cancelled`/`failed` 说明其实已跑完，不误报中断）。②**状态判定基准 = 本进程启动时刻**：心跳早于我们启动 ⇒ 那个进程已不在 ⇒ 标 `interrupted`（`recoverStaleTasks()` 启动时扫一遍；`loadTaskForResume()` 读时再判一次）。刻意不用「心跳超过 N 秒算过期」：那个阈值必须大于模型长调用的合法静默，于是「刚重启的头两分钟」会被误判成「还在别处跑」。退出钩子（SIGINT/SIGTERM）会就地标中断；Windows 上 pm2 是强杀（钩子不执行），故恢复扫描是主路径。③**无进展看门狗**：`CHAT_TASK_STALL_MS`（默认 = 模型超时 + 5 分钟）内无实质事件 → 主动发 `error`（`CHAT_TASK_STALLED`）+ abort + 收束；判据只认实质事件，传输层 `ping` 不算进展。④**防跨轮串流**：`POST /chat/stream` 回传 `X-Chat-Task-Id`，续传带 `?taskId=` 校验（不匹配即 404）；前端 `resumeRun()` 指数退避 3 次自动续传、`attachRunningTask()` 打开对话即接回正在跑的轮次，且续传成功**静默**接上（不插提示、不加噪音）。
- **边界（务必按这个口径理解）**：能续的是「**流的读取**」，不是「**这一轮的执行**」。进程一没，模型循环随之消失；这里刻意不重放、不重试模型调用（重放可能重复副作用），只保证「已产出的内容不丢 + 状态诚实」。真要「重启后继续跑”，得把 Agent 循环本身做检查点（另一件事，且要单独解决副作用幂等）。
- **验证**：`tests/task-resume.test.ts`（7 例：增量不入缓冲 / 打号 / 快照替换 / 留档过期即可回收 / 留档可关闭 / 看门狗主动收口并丢弃迟到产物）+ `tests/task-persistence.test.ts`（6 例：留档读写 / 僵尸判定 / 恢复扫描 / 回放游标 / 看门狗判据 / 退出时标中断并落档）；实例验证：真实服务断开续传（游标续读 + 快照 160 字）、强杀重启后续传拿到 `CHAT_TASK_INTERRUPTED`、确定性夹具 9/9（含 `taskId` 不匹配 404）。全量 19 文件 / 119 例、`tsc`、`vue-tsc` 全绿。
